"""Helpers for the Mean Reversion SHADOW (Entry-VaR) monitoring page.

Mirrors mpt_monitor_helpers.py but reads from Firebase folder `mpt_shadow_v1/`
and the local state dir `BloombergCOT/analytics/monitor_state/mpt_shadow_v1/`.

Same candidate universe as prod (top-1 per family + IP), but MPT objective was
solved on entry-VaR-scaled $-P&L covariance and sizing at deployment uses
VaR99 FROZEN at each trade's entry day.
"""
...
FIREBASE_FOLDER = "mpt_shadow_v1"

BLOOMBERG_COT = Path(
    r"c:\Users\Jia Shang\OneDrive - Hotei Capital\Desktop\BloombergCOT"
)
LOCAL_MODE = BLOOMBERG_COT.exists()
STATE_DIR = BLOOMBERG_COT / "analytics" / "monitor_state" / "mpt_shadow_v1"
TRADE_LOG_FP = BLOOMBERG_COT / "analytics" / "live_trade_log_shadow.csv"
REFRESH_SCRIPT = BLOOMBERG_COT / "analytics" / "_monitor_refresh_shadow.py"
DATA_REFRESH_SCRIPT = BLOOMBERG_COT / "analytics" / "refresh_18m.py"
SYNC_SCRIPT = BLOOMBERG_COT / "analytics" / "_sync_shadow_to_firebase.py"
...
def load_backtest_baseline_daily_pnl(year: int = YEAR) -> pd.Series:
    """The shadow backtest's own daily_pnl sheet, sliced to `year`. From Firebase."""
    raw = _fetch_bytes(f"{FIREBASE_FOLDER}/portfolio_MPT_entry_var_SHADOW.xlsx")
    if raw is None:
        return pd.Series(dtype=float)
    try:
    ...
    s = s.set_index("Date")
    # Shadow xlsx uses 'daily_pnl'; prod uses 'portfolio_daily_pnl'. Handle both.
    col = "daily_pnl" if "daily_pnl" in s.columns else "portfolio_daily_pnl"
    return s.loc[s.index.year == year, col]
...
def derive_status_row(pick: dict) -> dict:
    ...
    return {
        ...
        "var99_bbl_live": pick.get("var99_bbl_live", pick.get("var99_bbl")),
        "var99_bbl_entry": pick.get("var99_bbl_entry"),
        "current": ew,
        "median": med,
        "lower": lower,
        "upper": upper,
