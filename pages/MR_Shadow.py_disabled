"""SHADOW (Entry-VaR) monitoring page.

Same candidate universe as production MPT-7 v2 (top-1 per family + IP), but
picks are re-solved on covariance of entry-VaR-scaled $-P&L, and sizing at
deployment uses VaR99 FROZEN at each trade's entry day.

Status table shows BOTH VaR columns:
- Entry VaR99 → frozen at trade entry; NaN when flat. Drives sizing.
- Live VaR99  → refreshed every cycle. Display-only.

Data source: `BloombergCOT/analytics/monitor_state/mpt_shadow_v1/` (refreshed by
`_monitor_refresh_shadow.py`).
"""
...
from utils.mpt_monitor_helpers_shadow import (
...
st.set_page_config(layout="wide", page_title="MR Shadow (Entry-VaR)")
st.title("Mean Reversion - Shadow (Entry-VaR frozen) - Y2026")
st.caption(
    "Same universe as production MPT-7 v2 (top-1 per family + IP). MPT solved on "
    "entry-VaR-scaled $-P&L covariance. Position size at deployment uses VaR99 "
    "frozen at trade entry day. Live VaR99 (right column below) is display-only."
)
...
display_df = status_df[[
    ...
    "var99_bbl_entry", "var99_bbl_live",
    "current", "median",
...
display_df.columns = ["Diff", "Shape", "Formula", "Params", "Weight",
                      "Entry VaR99 ($/bbl)", "Live VaR99 ($/bbl)",
                      "Current", "Median",
...
styled = (display_df.style
          .format({"Weight": "{:.1%}",
                   "Entry VaR99 ($/bbl)": _fmt_var,
                   "Live VaR99 ($/bbl)": _fmt_var,
...
st.dataframe(
    ...
    column_config={
        ...
        "Entry VaR99 ($/bbl)": st.column_config.Column(
            width="small",
            help="Frozen at trade entry day. NaN when flat. Drives position size."),
        "Live VaR99 ($/bbl)": st.column_config.Column(
            width="small",
            help="Trailing 252-BD VaR99 as of latest bar, refreshed each cycle. Display-only."),
